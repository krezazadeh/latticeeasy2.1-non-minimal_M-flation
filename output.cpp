...
The functions here which should be called externally are output_parameters(double time) and save(int force).
...
void meansvars(int flush)
{
  ...
  for(fld=0;fld<nfldsout;fld++)
  {
    ...
    fprintf(means_," %e",f0*(av*rescaling)/pow(a,1.5));

    // fprintf(vars_," %e",var*pw2(rescaling));
    fprintf(vars_," %e",pw2(f0)*(var*pw2(rescaling))/pow(a,3.0));

    // KR
    if (fld == 1) {
        avCHIph2 = pw2(f0)*(var*pw2(rescaling))/pow(a,3.0);
    }
...
void spectra()
{
  ...
  int numbins=(int)(sqrt((double)NDIMS)*(N/2))+1; // Actual number of bins for the number of dimensions
  ...
  double norm1=2.*pi*pw2(L)/pw2(pw2(dx)); // Normalization designed to match spectra to 3D results
  double norm2=pw2(pw2((double)N))/2./pi; // A different normalization is used for number and energy spectra
#elif NDIMS==2
  int i,j,k,py,pz; // py and pz are components of momentum in units of grid spacing
  double norm1=pi*L/pw2(dx); // Normalization designed to match spectra to 3D results
  double norm2=pw2((double)N)/2.; // A different normalization is used for number and energy spectra
  double fnyquist[2*N],fdnyquist[2*N]; // Used by FFT routine to store modes with k=nyquist
  ...
  double fnyquist[N][2*N],fdnyquist[N][2*N]; // Used by FFT routine to store modes with k=nyquist
...
#if NDIMS==1
  fftr1((double *)f[fld],N,1); // Transform field values to Fourier space
  fftr1((double *)fd[fld],N,1); // Transform field derivatives to Fourier space
#else
  fftrn((double *)f[fld],(double *)fnyquist,NDIMS,arraysize,1); // Transform field values to Fourier space
  fftrn((double *)fd[fld],(double *)fdnyquist,NDIMS,arraysize,1); // Transform field derivatives to Fourier space
#endif
  ...
  for(k=1;k<N/2;k++)
  {
    ...
    if(omegasq > 0.) // Guard against doubleing point errors
    {
  ...
  for(k=0;k<=N/2;k+=N/2) // "Loop" over the two values k=0 and k=N/2
  {
    ...
    omegasq = fabs(pw2(dp*pmagnitude) + mass_sq[fld]); // Calculate omega_p^2
    if(pmagnitude==0.) // Avoid doubleing point errors
      continue;
    if(omegasq > 0.) // Guard against doubleing point errors
    {
      omega = sqrt(omegasq); // Omega = Sqrt(p^2 + m^2)
...
#if NDIMS==1
    fftr1((double *)f[fld],N,-1);
    fftr1((double *)fd[fld],N,-1);
#else
...
void energy()
{
  ...
  for(fld=0;fld<nflds;fld++) // Note that energy is output for all fields regardless of the value of nfldsout
  {
    ...
    vard = vard/(double)gridsize;
    ffd = ffd/(double)gridsize;
