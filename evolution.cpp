...
The function evolve_scale(double d) advances the value of the scale factor. The behavior of this function depends on the type of expansion being used. For no expansion (0) the function does nothing. For power-law expansion (1) the function calculates a and its derivatives at time t according to a preset formula. For consistent expansion (2) the function calculates the second derivative of a and uses it to advance adot by an interval d. In this case the scale factor itself is advanced in the function evolve_fields().
The function evolve_fields(double d) advances the field values (f) and the scale factor (a) by a time interval d assuming that the field derivatives (fd) and the scale factor derivative (ad) are known at a time in the middle of that interval.
The function evolve_derivs(double d) advances the field derivatives by a time interval d. It assumes f is known at the midpoint of that interval, uses it to calculate the second derivative, and uses that to advance the first derivatives. It also calls evolve_fields() to advance the scale factor.
*/
...
inline double lapl(int fld, INDEXLIST)
{
#if NDIMS==1
...
inline double laplb(int fld, INDEXLIST)
{
#if NDIMS==1
...
double gradient_energy(int fld)
{
  int i=0,j=0,k=0; // The initial values are just put in to make the compiler happy
  double gradient=0.;
  double norm=1./pw2(dx); // Converts the output of lapl() to an actual Laplacian
...
{
  int term ,fld;
  double grad_energy=0.,pot_energy=0.; // Gradient and potential energies of fields
...
void evolve_derivs(double d)
{
  ...
  double laplnorm = 1./pw2(dx)/pow(a,2.*rescale_s+2.); // Set coefficient for laplacian term in equations of motion. The dx^2 converts the output of lapl() to a laplacian and the scale factor term accounts for model dependent rescalings of the equations of motion.
