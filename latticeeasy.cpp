...
#if NDIMS==1
double f[nflds][N],fd[nflds][N];
#elif NDIMS==2
double f[nflds][N][N],fd[nflds][N][N];
#else
double f[nflds][N][N][N],fd[nflds][N][N][N];
#endif
double t,t0; // Current time and initial time (t0=0 unless the run is a continuation of a previous one)
double a=1.,ad=0.,ad2=0.,aterm=0.; // Scale factor and its derivatives (aterm is a combination of the others used in the equations of motion). Values are initialized to their defaults for the case of no expansion.
double hubble_init=0.; // Initial value of the Hubble constant
...
double rescaling=1.; // Rescaling for output. This is left as 1 unless the model file modifies it.
...
double model_vars[num_model_vars]; // Model-specific variables
