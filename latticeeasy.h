...
extern double rescaling; // Rescaling for output. This is left as 1 unless the model file modifies it.
...
#if NDIMS==1
extern double f[nflds][N],fd[nflds][N]; // Field values and derivatives
...
#elif NDIMS==2
extern double f[nflds][N][N],fd[nflds][N][N]; // Field values and derivatives
...
#elif NDIMS==3
extern double f[nflds][N][N][N],fd[nflds][N][N][N]; // Field values and derivatives
...
// evolution.cpp
double gradient_energy(int fld); // Calculate the gradient energy, <|Grad(f)|^2>=<-f Lapl(f)>, of a field
void evolve_scale(double d); // Calculate the scale factor and its derivatives
void evolve_fields(double d); // Advance the field values and scale factor using the first derivatives
void evolve_derivs(double d); // Calculate second derivatives of fields and use them to advance first derivatives. Also calls evolve_scale().
// output.cpp
...
// ffteasy.cpp
void fftr1(double f[], int N, int forward); // Do a Fourier transform of a 1D array of real numbers. Used when NDIMS=1.
