...
#if NDIMS==1
  static double norm = rescale_A*rescale_B*pow(L/pw2(dx),.5)/sqrt(4.*pi);
#elif NDIMS==2
  static double norm = rescale_A*rescale_B*L/pw2(dx)/sqrt(2.*pi);
#elif NDIMS==3
  static double norm = rescale_A*rescale_B*pow(L/pw2(dx),1.5)/sqrt(2.);
#endif
  static double hbterm = hubble_init*(rescale_r-1.);
...
void initialize()
{
  ...
  double dp2=pw2(2.*pi/L); // Square of grid spacing in momentum space
  double mass_sq[nflds]; // Effective mass squared of fields
  double initial_field_values[nflds]; // Initial value of fields (set to zero unless specified in parameters.h)
  double initial_field_derivs[nflds]; // Initial value of field derivatives (set to zero unless specified in parameters.h)
  double fsquared=0.,fdsquared=0.,ffd=0.,pot_energy=0.; // Sum(field^2), Sum(field_dot^2), Sum(f*f_dot), and potential energy - used for calculating the Hubble constant
  FILE *old_grid_; // Used for reading in previously generated data as initial conditions
#if NDIMS==1
  int i,k;
  double pz; // Component of momentum in units of grid spacing
#elif NDIMS==2
  int i,j,k,jconj; // jconj is the index where conjugate modes to j are stored
  double py,pz; // Components of momentum in units of grid spacing
  ...
  for(fld=0;fld<nflds;fld++) // Set initial conditions for each field
  {
    ...
    if(seed>=0)
    {
      fftr1((double *)f[fld],N,-1); // Inverse Fourier transform of field
      fftr1((double *)fd[fld],N,-1); // Inverse Fourier transform of field derivatives
    }
#elif NDIMS==2
    // Loop over gridpoints.
    ...
    if(seed>=0)
    {
      fftrn((double *)f[fld],(double *)fnyquist,NDIMS,arraysize,-1); // Inverse Fourier transform of field
      fftrn((double *)fd[fld],(double *)fdnyquist,NDIMS,arraysize,-1); // Inverse Fourier transform of field derivatives
    }
#elif NDIMS==3
    // Loop over gridpoints.
